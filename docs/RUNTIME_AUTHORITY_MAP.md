...
| Configure provider fallback | Resolver + RRM; ProviderManager invocation binding | Bridge selects default/fallback flags; Kernel routes by legacy mode | Resolver selects eligible provider; manager invokes only selection | Not started | Zero-provider, unhealthy provider, explicit fallback and provenance tests. |
...
| Author response meaning | Conversation/Mission runtime + CognitiveResponseAssembler | Bridge creates text/status/mode/confidence/provenance/Mission meaning | Services return CognitiveResponse; assembler governs; bridge serializes | Not started | All major sources produce consistent status, epistemic state, provenance, limitations. |
